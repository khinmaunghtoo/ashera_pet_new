...
class NotifyCardLike extends StatefulWidget {
  final NotifyModel model;
  const NotifyCardLike({super.key, required this.model});

  @override
  State<StatefulWidget> createState() => _NotifyCardLikeState();
}

class _NotifyCardLikeState extends State<NotifyCardLike> {
  ...
  Future<Tuple<bool, String>> getPostImage(int postId) async {
    ...
    if (r.i1!) {
      //log('getPostImage: $postId ${r.i1} ${r.i2}');
      if (r.i2 == '""') {
        //已刪除貼文
        ...
        if (Utils.imageFileVerification(imageUrl.first)) {
          ...
          return r;
        } else if (Utils.videoFileVerification(imageUrl.first)) {
